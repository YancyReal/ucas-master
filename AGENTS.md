...
论文涉及的实际项目代码不在本仓库内，而在 `/workspace/aosp/art`；benchmark、实验结果和方案报告在 `/workspace/aosp/tmp/MyBenchMark`。其中，仓库说明中的 `/workspace/aosp/` 是逻辑路径，对应 `la55-aliyun` 机器上的 `/home/yanxi/loongson/aosp15.la/`；因此 `/workspace/aosp/art` 实际对应 `la55-aliyun:/home/yanxi/loongson/aosp15.la/art`，`/workspace/aosp/tmp/MyBenchMark` 实际对应 `la55-aliyun:/home/yanxi/loongson/aosp15.la/tmp/MyBenchMark`。修改论文中的实现细节、实验流程、性能结论前，先核对这两个目录下的说明文档与现有数据，不要把 `ucasthesis/` 误当成源码或结果归档仓库。
...
- 错误纠正规则：若当前本地环境不存在 `/workspace/aosp/...`，应先通过 `ssh la55-aliyun` 到 `/home/yanxi/loongson/aosp15.la/...` 对应目录查询，不得在未核对远端路径映射前直接判定外部代码或实验目录缺失。
- 错误纠正规则：给出论文审查方案时，必须将“用词用语是否规范”作为独立审查维度明确列出，不能仅检查结构、技术内容和实验设计。
- 图表工作流规则：论文机制图必须按“逐幅重画 → 用户先审 drawio → 通过后再导出并替换论文图片”的顺序执行，禁止未审先入正文。
- 图表交付规则：除非用户明确要求“放到论文里/导出 PDF 并替换引用图”，否则默认仅交付 drawio 源图与 sidecar，等待用户审阅通过后再执行导出与编译。
- 图表文案规则：图中仅展示结构关系与中性术语，不写“证明目标/证据闭环”等解释性结论句；解释性文字放正文。
- 图表可读性规则：优先保证最终 PDF 渲染可读性，字号与模块尺寸宁大勿小；若渲染不清晰，先整体放大字体再微调布局。
- 图表风格规则：机制示意图优先采用白底+浅色分区（panel）+简洁箭头的期刊风格，避免花哨装饰与过度占位连线。
