...
\texttt{startup.helloworld} 之所以被选为主目标工作负载，是因为对照版本 flamegraph 中 verifier、
...
两个文件中。正文将其写成阶段链，而非单点容器优化，正是因为对照版本 flamegraph 显示 container probing、
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcccc}
        \hline
        \textbf{阶段} & \textbf{3 轮原始分数（\texttt{ops/m}）} & \textbf{均值} & \textbf{标准差} & \textbf{相对对照版本收益} \\
        \hline
        对照版本 & \texttt{36.74, 36.74, 37.38} & \texttt{36.9533} & \texttt{0.3017} & \texttt{+0.00\%} \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcccc}
        ...
        \textbf{阶段} & \textbf{3 轮原始分数（\texttt{ops/m}）} & \textbf{均值} & \textbf{标准差} & \textbf{相对对照版本收益} \\
        \hline
        对照版本 & \texttt{82.37, 77.93, 82.90} & \texttt{81.0667} & \texttt{2.2285} & \texttt{+0.00\%} \\
