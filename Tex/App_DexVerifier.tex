...
本附录对应第 6.3 节中的启动期校验快照（\texttt{Snapshot-C}），用于保留 \texttt{DexFileVerifier}
阶段链的详细可追溯材料。正文仅保留主结论和主要图表，而将更细的 flamegraph 依据、源码位置与原始数据表
集中放在本附录中。

\texttt{startup.helloworld} 之所以被选为主目标 workload，是因为 baseline flamegraph 中 verifier、
...
两个文件中。正文将其写成阶段链，而非单点容器优化，正是因为 baseline flamegraph 显示 container probing、
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcccc}
        \hline
        \textbf{阶段} & \textbf{3 轮原始分数（\texttt{ops/m}）} & \textbf{均值} & \textbf{标准差} & \textbf{相对 baseline} \\
        \hline
...
表~\ref{tab:app-dexverifier-startup} 与表~\ref{tab:app-dexverifier-lu} 共同支持第 6.3 节的写法：
